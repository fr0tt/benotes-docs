...
#- benotes_storage:/var/www/storage</code></pre>
    ...
    <p>Short answer: You only have to make sure that the <code>storage</code> directory is owned by www-data (see <a
            href="#volumes">Volume section</a> above).</p>
    <p>Long answer: If you keep your existing docker-compose.yml you are going to use bind mounts
        ...
        created volume which might be located at
        <code>/var/lib/docker/volumes/yourBenotesInstallationDirectoryName_benotes_storage/_data</code>
        - next to the database volume. (You can also use <code>docker volume ls</code> and
